...
- Removed the private RC4/MD5 PDF password-protection implementation and its
  dormant AES placeholder. PaperRune no longer exposes weak or partial PDF
  encryption machinery.
- Extracted every existing-PDF capability into the independent PDFRune module:
  parsing, page import, inspection, CDR, signing, signature verification,
  characterization, and final-byte verification. Removed the PaperRune APIs,
  CLI/Studio routes, and compatibility wrappers instead of deprecating them.
- Made serialized PDF output terminal inside PaperRune. Studio accessibility,
  review, and delivery evidence now comes from the retained immutable plan;
  hosts may pass generated bytes to PDFRune after export.
- Simplified `paperd` to transient in-process authoring state. Removed its
  persistence, transport, approval workflow, separate revision domains, and
  the `paper workflow` command; durable history now belongs to Git or the host.
- Removed NilAway from the pinned tool module and CI/release gates. The
  contributor gate retains formatting, tests, race detection, `go vet`,
  golangci-lint, gosec, coverage, and module verification.
