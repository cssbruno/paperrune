...
const testProfile = `{
  ...
  "minimum_samples":2,
  "benchmarks":{
    "BenchmarkPaperEngineEndToEndPaper":{"max_median_ns_per_op":120,"max_bytes_per_op":210,"max_allocs_per_op":12},
...
BenchmarkPaperEngineEndToEndPaper-8 10 100 ns/op 200 B/op 10 allocs/op
BenchmarkPaperEngineEndToEndPaper-8 10 110 ns/op 201 B/op 11 allocs/op
...
func TestValidateIsDeterministicHostAwareAndUsesNamedBudgets(t *testing.T) {
	...
	if first.Results[0].Name != "BenchmarkPaperEngineEndToEndPaper" || first.Results[0].MedianNSPerOp != 110 ||
...
func TestValidateRejectsHostSampleMetricAndBudgetDrift(t *testing.T) {
	tests := []struct{ name, report, want string }{
		...
		{"samples", strings.Replace(testReport, "BenchmarkPaperEngineEndToEndPaper-8 10 110 ns/op 201 B/op 11 allocs/op\n", "", 1), "requires at least 2"},
...
func TestCheckedAppleM2CalibrationProfileIsValid(t *testing.T) {
	...
	if err != nil || profile.Name != "apple-m2-go1.26-paper-only-v2" || len(profile.Benchmarks) != 2 {
